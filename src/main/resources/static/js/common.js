let login = (event) => {//let login=function(event){};と同義
	event.preventDefault();
	let jsonString = {
		'userName':$('input[name=userName]').val(),
		'password':$('input[name=password]').val()
	};
	$.ajax({//ajaxを呼び出し、IndexControllerに追加したloginメソッドにアクセス。
		type:'POST',
	...
	//jqueryのajaxを呼び出し、IndexControllerに追加したloginメソッドにアクセス
...
let addCart = (event) => {
	...
	$('.removeBtn').on('click',removeCart);//これはindexの方じゃないのか？
};
